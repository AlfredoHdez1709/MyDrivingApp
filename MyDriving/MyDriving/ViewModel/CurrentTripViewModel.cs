...
using MvvmHelpers;
//1 using Plugin.Media.Abstractions;
//2 using Plugin.Media;
using Plugin.DeviceInfo;
using MyDriving.Services;
using Acr.UserDialogs;
//3 using MyDriving.Utils.Helpers;
//4 using MyDriving.AzureClient;
//5 using MyDriving.Utils.Interfaces;

namespace MyDriving.ViewModel
{
    //TODO: Descomentar cuando se agreguen los servicios respectivos (35)
    public class CurrentTripViewModel : ViewModelBase
    {
        ...
        public CurrentTripViewModel()
        {
            ...
            {
                //6 UserId = Settings.Current.UserUID,
                Points = new ObservableRangeCollection<TripPoint>()
            };
            photos = new List<Photo>();
            fuelConsumptionRate = 0;
            //7 FuelConsumptionUnits = Settings.MetricUnits ? "Liters" : "Gallons";
            //8 DistanceUnits = Settings.MetricDistance ? "Kilometers" : "Miles";
        ...
        //9 public IMedia Media => CrossMedia.Current;
        ...
        public async Task<bool> StartRecordingTrip()
        {
            ...
            //10 await AzureClient.AzureClient.CheckIsAuthTokenValid();
            ...
            try
            {
                if (CurrentPosition == null)
                {
                        ...
                        CrossDeviceInfo.Current.Platform == Plugin.DeviceInfo.Abstractions.Platform.WindowsPhone)
                    {
                        UserDialogs.Instance.Toast(
                            new ToastConfig("Waiting for current location.")
                            {
                ...
                IsRecording = true;
                //11 Logger.Instance.Track("StartRecording");
                //add start point
        ...
        public async Task<bool> SaveRecordingTripAsync(string name = "")
        {
            ...
            {
                //13 Logger.Instance.Track("Attempt to save a trip with no points!");
                return false;
            }
            IsBusy = true;
            ...
            try
            {
                ...
                //14 ProgressDialogManager.LoadProgressDialog("Saving trip...");
                ...
                */
                ...
                //16 await StoreManager.TripStore.InsertAsync(CurrentTrip);

                foreach (var photo in photos)
                {
                    photo.TripId = CurrentTrip.Id;
                    //17 await StoreManager.PhotoStore.InsertAsync(photo);
                }
                ...
                NeedSave = false;
                //18 Logger.Instance.Track("SaveRecording");
                return true;
            }
            catch (Exception ex)
            {
                //19 Logger.Instance.Report(ex);
            }
            finally
            {
                ...
                //20 ProgressDialogManager.DisposeProgressDialog();
            }
            //21 Logger.Instance.Track("SaveRecording failed for trip " + tripId);
            return false;
        }
        ...
        async Task ExecuteTakePhotoCommandAsync()
        {
            try
            {
                ...
                */
                ...
                var locationTask = Geolocator.GetPositionAsync(TimeSpan.FromMilliseconds(2500));
